...
class DDPG(AbstractLearner):
    '''
    Deep Deterministic Policy Gradient learner.
    '''
    ...
    def __init__(self,**kwargs):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('critic'):
                with tf.variable_scope('update'):
                    self.qc = self.qvalue_net(self.tfs,self.tfa)
                with tf.variable_scope('update',reuse=True):
                    self.qa = self.qvalue_net(self.tfs,self.a)
                with tf.variable_scope('target'):
            ...
            self.closs = tf.reduce_mean(tf.square(self.tfy - self.qc))
            self.ctrain_op = tf.train.AdamOptimizer(c_lr).minimize(self.closs,var_list=q_params)

            self.aloss = -tf.reduce_mean(self.qa)
            self.atrain_op = tf.train.AdamOptimizer(a_lr).minimize(self.aloss,var_list=a_params)

            self.params = a_params + q_params

            self.update_op = [pp.assign((1-self.tau)*pp+self.tau*p) for p, pp in zip(a_params + q_params,ap_params + qp_params)]
