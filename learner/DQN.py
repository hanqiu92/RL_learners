...
class DQN(AbstractLearner):
    '''
    Deep Q Network learner with experience replay and target network.
    '''
    ...
    def learn(self):
        ...
        if state is not None:
            ...
            v = self.sess.run(self.q_target, {self.tfs: state_for_q, self.tfa: action_for_q}).reshape((len(next_state),len(self.actions)))
            q = reward + self.gamma * v.max(axis=-1)
            self.sess.run(self.train_op, {self.tfs: state, self.tfa: action, self.tfq: q})
            self.learn_step_counter += 1

            super(DQN,self).learn()
